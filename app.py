...
#     potential_columns = ['sol_id', 'branch_lap_dat_may', 'branch_code', 'brcd', 'ma_cn', 'chinhanh', 'SOL_ID_FROM', 'sol_id_from', 'SOL_ID']
...
#                 file_name=f"Filter"_{chi_nhanh}.xlsx",
...
if uploaded_file:
    ...
    if found_col:
        ...
        if chi_nhanh:
            # Lọc dữ liệu
            df_ftp_filtered = df_tt[
                df_tt[found_col]
                .astype(str)
                .str.upper()
                .str.contains(chi_nhanh, na=False)
            ]
            ...
            # Lấy tên file gốc (không có .xlsx)
        ...
        st.error(
            "❌ Không tìm thấy cột nào liên quan đến Chi nhánh "
            "(BRCD, BRANCH_CODE, SOL_ID...). Vui lòng kiểm tra lại file."
        )
